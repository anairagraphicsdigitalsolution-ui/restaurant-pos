...
public class AnairaLocalDbPlugin extends Plugin {
    ...
    @PluginMethod
    public void setSyncSession(PluginCall call) {
        String restaurantId = call.getString("restaurantId", "");
        String token = call.getString("token", "");
        String apiBase = call.getString("apiBase", "https://www.anairapos.in");
        if (restaurantId.isEmpty() || token.isEmpty()) { call.reject("restaurantId and token are required"); return; }
        try {
            AnairaSyncSession.save(getContext(), restaurantId, token, apiBase);
            AnairaSyncWorker.schedule(getContext());
            AnairaSyncWorker.runNow(getContext());
            JSObject out = new JSObject(); out.put("success", true); out.put("scheduled", true); call.resolve(out);
        } catch (Exception e) { call.reject(e.getMessage()); }
    }

    @PluginMethod
    public void clearSyncSession(PluginCall call) {
        AnairaSyncSession.clear(getContext());
        call.resolve();
    }
