...
export default function AddTable(){
 ...
 async function addTable(e){
   e.preventDefault()
   if(!tableName .trim() || !Number.isInteger(Number(tableName)) || Number(tableName) < 1){setMessage("Enter a valid table number.");return}
   ...
   try {
     ...
     const res = await fetch("/api/dashboard-add-table", {
       method:"POST",
       headers:{"Content-Type":"application/json", ...(session?.access_token ? {Authorization:`Bearer ${session.access_token}`} : {})},
       body:JSON.stringify({table_number: Number(tableName)})
     })
     const result = await res.json()
     if(!res.ok || !result.success) throw new Error(result.error || "Unable to add table")
     setMessage("Table added successfully.")
     setTableNumber("")
   } catch(error) {
     console.error(error)
     setMessage(error?.message || "Unable to add table. Please try again.")
   } finally {
     setLoading(false)
   }
 }
 return <main className="utility-page"><div className="utility-shell"><div className="eyebrow">FLOOR MANAGEMENT</div><h1>Add Table</h1><p className="lead">Create a table that can be used for dine-in orders and floor management.</p><form className="utility-card" onSubmit={addTable}><div className="utility-icon">🪑</div><label>Table name<input value={tableName} onChange={e=>setTableName(e.target.value)} type="number" min="1" step="1" placeholder="e.g. 1" autoFocus/></label>{message&&<div className="msg">{message}</div>}<button disabled={loading || authLoading || !restaurantId}>{loading?"Adding…":authLoading?"Loading restaurant…":"＋ Add Table"}</button></form></div><style jsx global>{utilityCss}</style></main>
}
