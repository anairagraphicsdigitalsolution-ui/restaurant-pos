...
export default function CombosPage() {
  ...
  const [fixedVariantIds, setFixedVariantIds] = useState({})
  const [choiceVariantIds, setChoiceVariantIds] = useState({})
  ...
  async function load(restaurantId = rid) {
    ...
    const [{ data: menu }, { data: comboRows }] = await Promise.all([
      supabaseCloud.from("menu_items").select("id,name,price,category,image,item_type,menu_variants(id,name,price_delta,active)").eq("restaurant_id", restaurantId).order("category").order("name"),
  ...
  function editCombo(combo) {
    ...
    const fixedVariants = {}
    ;(cfg.items || []).forEach(row => { if (row.variant_id) fixedVariants[row.item_id] = row.variant_id })
    const choiceVariantMap = {}
    ;(cfg.groups?.[0]?.options || []).forEach(row => { if (row.variant_id) choiceVariantMap[row.item_id] = row.variant_id })
    setFixedVariantIds(fixedVariants)
    setChoiceVariantIds(choiceVariantMap)
  ...
  function reset() {
    setEditingId(""); setForm(emptyForm); setSelectedItems({}); setSelectedOptions([]); setOptionDeltas({}); setFixedVariantIds({}); setChoiceVariantIds({}); setMessage("")
  }
  ...
  async function save() {
    ...
    const comboConfig = form.mode === "fixed"
      ? { mode: "fixed", items: Object.entries(selectedItems).filter(([, qty]) => qty).map(([item_id, quantity]) => { const variant_id=fixedVariantIds[item_id] || null; const variant=(items.find(i=>i.id===item_id)?.menu_variants||[]).find(v=>v.id===variant_id); return { item_id, quantity: Number(quantity), variant_id, variant_name: variant?.name || null } }) }
      : { mode: "choice", groups: [{ name: form.groupName.trim() || "Choose an option", min: Number(form.min || 1), max: Number(form.max || 1), options: selectedOptions.map(item_id => { const variant_id=choiceVariantIds[item_id] || null; const variant=(items.find(i=>i.id===item_id)?.menu_variants||[]).find(v=>v.id===variant_id); return { item_id, price_delta: Number(optionDeltas[item_id] || 0), variant_id, variant_name: variant?.name || null } }) }] }
  ...
  return <main className="combo-page">
    ...
    <section className="builder">
      ...
      {form.mode === "fixed" ? <div className="item-grid">{filteredItems.map(item => { const variants=(item.menu_variants||[]).filter(v=>v.active!==false); const selected=!!selectedItems[item.id]; return <div className={`item-pick-wrap ${selected ? "selected" : ""}`} key={item.id}><button type="button" className={`item-pick ${selected ? "selected" : ""}`} onClick={() => toggleFixed(item.id)}><span>{selected ? "✓" : "＋"}</span><div><b>{item.name}</b><small>{item.category || "Other"} · ₹{item.price}</small></div></button>{selected && variants.length>0 && <label className="variant-select">Variant<select value={fixedVariantIds[item.id] || ""} onChange={e=>setFixedVariantIds(p=>({...p,[item.id]:e.target.value||null}))}><option value="">Base item</option>{variants.map(v=><option key={v.id} value={v.id}>{v.name} · +₹{Number(v.price_delta||0).toFixed(2)}</option>)}</select></label>}</div>})}</div> : <div className="choice-box"><div className="choice-fields"><input value={form.groupName} onChange={e => setForm({...form,groupName:e.target.value})} placeholder="Choice group name" /><input type="number" min="1" value={form.min} onChange={e => setForm({...form,min:e.target.value})} placeholder="Min" /><input type="number" min="1" value={form.max} onChange={e => setForm({...form,max:e.target.value})} placeholder="Max" /></div><div className="item-grid">{filteredItems.map(item => { const chosen=selectedOptions.includes(item.id); const variants=(item.menu_variants||[]).filter(v=>v.active!==false); return <div className={`item-pick ${chosen ? "selected" : ""}`} key={item.id}><button type="button" className="item-pick-main" onClick={() => setSelectedOptions(prev => prev.includes(item.id) ? prev.filter(id => id !== item.id) : [...prev, item.id])}><span>{chosen ? "✓" : "＋"}</span><div><b>{item.name}</b><small>{item.category || "Other"} · ₹{item.price}</small></div></button>{chosen && <div className="choice-extra"><label className="delta-field">Extra ₹<input type="number" step="0.01" value={optionDeltas[item.id] ?? 0} onChange={e => setOptionDeltas(prev => ({...prev,[item.id]:e.target.value}))} /></label>{variants.length>0 && <label className="variant-select">Variant<select value={choiceVariantIds[item.id] || ""} onChange={e=>setChoiceVariantIds(p=>({...p,[item.id]:e.target.value||null}))}><option value="">Base item</option>{variants.map(v=><option key={v.id} value={v.id}>{v.name} · +₹{Number(v.price_delta||0).toFixed(2)}</option>)}</select></label>}</div>}</div>})}</div></div>}
