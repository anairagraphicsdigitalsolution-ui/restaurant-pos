...
export async function GET(req) {
  try {
    ...
    if (!restaurantId) throw new Error("Restaurant profile not found")
    const { data, error } = await supabaseCloudAdmin.from("plugin_settings").select("config").eq("restaurant_id", restaurantId).eq("plugin_code", "whatsapp-invoice").maybeSingle()
...
export async function POST(req) {
  try {
    ...
    const { data: current } = await supabaseCloudAdmin.from("plugin_settings").select("config").eq("restaurant_id", restaurantId).eq("plugin_code", "whatsapp-invoice").maybeSingle()
    ...
    const { error } = await supabaseCloudAdmin.from("plugin_settings").upsert({ restaurant_id: restaurantId, plugin_code: "whatsapp-invoice", config: incoming }, { onConflict:"restaurant_id,plugin_code" })
    if (error) throw error
