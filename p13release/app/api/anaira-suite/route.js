  ...
  try {
    ...
    if (action === "payment") {
      ...
      const { data, error } = await supabaseCloudAdmin.rpc("p0_record_payment", {
        p_restaurant_id: rid, p_order_id: body.order_id, p_amount: amount,
      ...
      if (error) throw error
      await audit(rid, user.id, "payment.recorded", "order", body.order_id, { amount, method, p0: true })
      return NextResponse.json({ success: true, data })
    }
    if (action === "split") {
    ...
    if (action === "refund") {
      ...
      if (error) throw error
      await audit(rid, user.id, "refund.created", "order", body.order_id, { amount, p0: true }, body.reason)
      return NextResponse.json({ success: true, data })
    }
    if (action === "void") {
    ...
    console.error("Anaira parity", error)
