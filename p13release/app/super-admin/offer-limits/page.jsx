...
export default function OfferLimitsPage(){
  ...
  useEffect(()=>{load()},[])
  async function load(){
    const {data:ps}=await supabaseCloud.from("plans").select("id,name").order("name")
    const {data:ls}=await supabaseCloud.from("plan_feature_limits").select("plan_id,monthly_limit").eq("plugin_code","offers")
    setPlans(ps||[])
  ...
  async function save(planId){
    ...
    const {error}=await supabaseCloud.from("plan_feature_limits").upsert({plan_id:planId,plugin_code:"offers",monthly_limit},{onConflict:"plan_id,plugin_code"})
