    ...
    <main className="subscription-page">
      <section className="hero">
        ...
        <div className="hero-actions">
          <Link href="/super-admin#restaurant-setup" className="add-restaurant">＋ Add Restaurant</Link>
        ...
        {loading ? <div className="empty">Loading subscriptions…</div> : restaurants.length === 0 ? <div className="empty">No restaurants found.</div> : (
          <div className="restaurant-list">
            {restaurants.map(restaurant => {
              const current = latestByRestaurant.get(restaurant.id)
              ...
              return (
                <article className="restaurant-row" key={restaurant.id}>
                  ...
                  <div className="plan-summary">
                    ...
                    <strong>{plan?.name || "Pending — No plan assigned"}</strong>
...
.eyebrow{font-size:11px;letter-spacing:2px;color:var(--primary);font-weight:900}.hero h1{margin:8px 0;font-size:clamp(32px,5vw,52px);letter-spacing:-.04em}.hero p{margin:0;color:var(--muted);line-height:1.7}.hero-note{display:inline-flex;margin-top:15px;padding:8px 12px;border-radius:999px;background:rgba(var(--warning-rgb,245,158,11),.08);border:1px solid rgba(245,158,11,.2);color:var(--warning);font-size:12px;font-weight:800}.hero-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.add-restaurant{display:inline-flex;align-items:center;justify-content:center;padding:12px 16px;border-radius:13px;background:var(--primary);color:#111;text-decoration:none;font-weight:900;transition:all .2s ease}.add-restaurant:hover{background:var(--warning);color:#111;transform:translateY(-2px);box-shadow:0 12px 28px rgba(var(--primary-rgb),.18)}.hero-stat{min-width:170px;padding:22px;border-radius:22px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);display:grid;gap:4px}.hero-stat span,.hero-stat small{color:var(--muted)}.hero-stat strong{font-size:38px;color:var(--primary)}
