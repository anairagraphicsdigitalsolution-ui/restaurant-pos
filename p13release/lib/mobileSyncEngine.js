  ...
  try {
    ...
    for (const entry of queue) {
      try {
        ...
        if (entry.entity === "orders") {
          ...
          pushed += 1
        } else if (entry.entity === "billing_finalize") {
          const key = entry.payload?.idempotency_key || `offline-billing:${entry.entity_id}:${entry.queue_id}`
          const response = await fetch("/api/billing/finalize", {
            ...
            body: JSON.stringify({ ...entry.payload, idempotency_key: key, order_id: entry.entity_id }),
          })
          const result = await response.json().catch(() => ({}))
          if (!response.ok || !result.success) throw new Error(result.error || "Billing reconciliation failed")
          if (result.bill) await mobileDbPut(restaurantId, "invoices", entry.entity_id, result.bill)
          await mobileDbQueueUpdate(entry.queue_id, { status: "synced", synced_at: new Date().toISOString(), last_error: null }, restaurantId)
          pushed += 1
        } else if (entry.entity === "restaurant_deliveries") {
          const key = entry.payload?.idempotency_key || `offline-delivery:${entry.entity_id}:${entry.queue_id}`
          const response = await fetch("/api/delivery", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, "x-idempotency-key": key },
            body: JSON.stringify({ ...entry.payload, idempotency_key: key }),
          })
          const result = await response.json().catch(() => ({}))
          if (!response.ok || result.success === false) throw new Error(result.error || "Delivery reconciliation failed")
          await mobileDbQueueUpdate(entry.queue_id, { status: "synced", synced_at: new Date().toISOString(), last_error: null }, restaurantId)
          pushed += 1
        } else {
    ...
    for (const event of pullResult.events || []) {
      ...
      if (local && isStale(row, local)) {
        const conflict = { restaurant_id: restaurantId, entity, entity_id: row.id, direction: "cloud_to_local", resolution: "local-newer", local, incoming: row, created_at: new Date().toISOString() }
        await mobileDbConflictPut(conflict).catch(() => {})
        await fetch("/api/mobile/sync/conflict", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(conflict) }).catch(() => {})
        continue
      }
      if (row.id) {
