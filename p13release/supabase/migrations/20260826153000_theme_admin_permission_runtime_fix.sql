-- Theme admin permission runtime repair.
-- Fixes legacy rows where the permission checkbox is TRUE but its scope was
-- left at "none". In that state the Admin UI correctly refused the change.
-- TRUE + no valid scope is normalized to BOTH; FALSE always means NONE.
...
SET config = jsonb_set(
  jsonb_set(
    ...
    CASE
      WHEN COALESCE((config->>'allow_admin_theme_changes')::boolean, false)
       AND LOWER(COALESCE(config->>'admin_theme_change_scope','none')) NOT IN ('restaurant','qr','both')
        THEN '"both"'::jsonb
      ELSE to_jsonb(LOWER(COALESCE(config->>'admin_theme_change_scope','none')))
    END,
    true
  ),
  '{allow_admin_theme_changes}',
  to_jsonb(COALESCE((config->>'allow_admin_theme_changes')::boolean, false)),
  true
)
...
BEGIN
  ...
  -- Backward-compatible repair: if the Super Admin enabled the checkbox but
  -- an old row still says "none", treat it as BOTH. The UI also normalizes it
  -- on save, and this RPC prevents a stale row from blocking the Admin.
  IF COALESCE(allow_theme,false)
     AND allowed_scope NOT IN ('restaurant','qr','both') THEN
    allowed_scope := 'both';
  END IF;
  ...
  -- The Admin cannot expand beyond the scope granted by Super Admin.
...
GRANT EXECUTE ON FUNCTION public.admin_save_restaurant_theme(uuid,jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_save_restaurant_theme(uuid,jsonb) TO service_role;
