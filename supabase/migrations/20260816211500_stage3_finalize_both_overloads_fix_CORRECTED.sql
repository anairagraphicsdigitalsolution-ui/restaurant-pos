-- ============================================================
-- STAGE 3 FINALIZE DEFINITIVE OUTER-JOIN LOCK FIX
-- ============================================================
-- The database currently contains TWO overloads of
-- stage3_finalize_order():
--
--   1) (uuid, text, numeric, uuid)            <- current 4-arg API
--   2) (uuid, uuid, text, numeric, uuid)      <- legacy 5-arg API
--
-- The 4-arg function was already patched, but the legacy 5-arg
-- overload still contained:
--
--   LEFT JOIN ... 
--   ...
--   FOR UPDATE
--
-- That unqualified FOR UPDATE locks the nullable side (menu_items)
-- of the LEFT JOIN and causes:
--
--   FOR UPDATE cannot be applied to the nullable side of an outer join
--
-- This migration patches BOTH overloads to:
--
--   FOR UPDATE OF oi
--
-- Nothing else in either function is changed.
-- ============================================================

CREATE OR REPLACE FUNCTION public.stage3_finalize_order(p_order_id uuid, p_payment_method text DEFAULT 'cash'::text, p_paid_amount numeric DEFAULT 0, p_offer_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_four = 0 OR v_five = 0 THEN
    RAISE EXCEPTION 'Finalize function verification failed (4-arg=%, 5-arg=%)', v_four, v_five;
  END IF;

  RAISE NOTICE 'Both stage3_finalize_order overloads patched successfully';
END
$$;
