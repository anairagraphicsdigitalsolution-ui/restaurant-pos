-- ============================================================
-- BILLING FINALIZE IDEMPOTENCY FIX
-- ============================================================
-- A successful finalize must be a one-click operation. If the browser
-- loses the response, refreshes, or a duplicate request reaches the server,
-- a second finalize request must return the already-finalized bill rather
-- than raising "Order is already paid" or creating another payment.
--
-- The authoritative 6-argument function is defined by
-- 20260824214500_loyalty_reward_billing.sql. That migration is patched in
-- the source tree as well; this migration is the production-safe redefinition
-- to run after the existing migration history.
-- ============================================================
...
BEGIN
  ...
  -- IMPORTANT: idempotent success. A retry of an already completed finalize
  -- returns the existing invoice/payment state and does not insert anything.
  IF COALESCE(v_order.payment_status, 'unpaid') = 'paid' THEN
    RETURN jsonb_build_object(
      ...
      'delivery_charge', COALESCE(v_order.delivery_charge, 0),
      'loyalty_reward_id', NULL,
      'loyalty_points_redeemed', 0,
      'loyalty_discount', 0,
      'total', COALESCE(v_order.total_amount, 0),
  ...
  -- Snapshot legacy prices and lock only order_items (not nullable joined rows).
  FOR v_item IN
  ...
  LOOP
    ...
    IF v_item.unit_price IS NULL THEN
      ...
      SET item_name = COALESCE(item_name, v_item.name),
  ...
  IF p_loyalty_reward_id IS NOT NULL THEN
    ...
    IF lower(COALESCE(v_loyalty_reward.reward_type, 'discount')) = 'percent' THEN
      v_loyalty_discount := LEAST(
        ...
        ROUND(
          GREATEST(v_subtotal - v_discount, 0)
          * LEAST(GREATEST(COALESCE(v_loyalty_reward.reward_value, 0), 0), 100) / 100,
          2
        )
      );
    ELSE
  ...
  IF v_delivery_charge = 0
     AND COALESCE(v_order.order_mode, v_order.source_type) = 'delivery' THEN
  ...
  IF COALESCE(v_restaurant.gst_enabled, true) THEN
    v_tax := ROUND(
      GREATEST(v_subtotal - v_discount, 0)
      * GREATEST(COALESCE(v_restaurant.gst_rate, 0), 0) / 100,
      2
    );
  END IF;

  v_total := ROUND(
    GREATEST(v_subtotal - v_discount, 0) + v_tax + v_delivery_charge,
    2
  );
  ...
  -- Keep an existing invoice number if one already exists. This prevents a
  -- retry or resumed billing flow from consuming another invoice sequence.
...
-- ------------------------------------------------------------
-- Manual discount-aware overload used by the Billing screen.
-- It keeps the existing 6-argument API intact for older callers.
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.stage3_finalize_order(
  p_actor_id uuid,
  p_order_id uuid,
...
BEGIN
  ...
  -- Manual billing discount takes precedence over the selected offer, matching
  -- the Billing screen. Loyalty reward, when selected, still stacks after it.
  IF v_manual_discount > 0 THEN
    IF v_manual_discount_mode = 'percent' THEN
      v_discount := LEAST(
        v_subtotal,
        ROUND(v_subtotal * LEAST(v_manual_discount, 100) / 100, 2)
      );
    ELSE
  ...
  RETURN jsonb_build_object(
    ...
    'manual_discount_mode', v_manual_discount_mode,
    'total', v_total,
    'paid_amount', v_paid,
    'payment_received', v_payment_received,
    'payment_status', v_payment_status,
