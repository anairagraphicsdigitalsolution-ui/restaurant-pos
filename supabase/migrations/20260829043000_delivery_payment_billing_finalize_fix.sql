-- Delivery payment is a pre-collected payment, not invoice finalization.
-- Keep the payment ledger as the source of truth, but allow Billing Finalize
-- to generate the missing invoice when the ledger is already fully paid.
...
BEGIN
  ...
  -- A payment may already have been collected by Delivery settlement.
  -- Billing Finalize must still continue so a missing invoice can be generated.
...
DO $$ BEGIN
  IF to_regprocedure('public.stage3_finalize_order(uuid,uuid,text,numeric,uuid,uuid,numeric,text)') IS NOT NULL THEN
    EXECUTE 'ALTER FUNCTION public.stage3_finalize_order(uuid,uuid,text,numeric,uuid,uuid,numeric,text) OWNER TO postgres';
  END IF;
END $$;
