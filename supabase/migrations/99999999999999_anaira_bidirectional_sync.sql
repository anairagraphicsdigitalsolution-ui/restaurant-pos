-- Anaira POS local/cloud sync capture.
-- This migration is safe to apply on both sides; apply-event sessions set app.sync_apply=true
-- so replicated writes do not create a feedback loop.
...
begin
  for r in select a.attname from pg_index i join pg_attribute a on a.attrelid=i.indrelid and a.attnum=any(i.indkey)
  where i.indrelid=format('%I.%I',_schema,_table)::regclass and i.indisprimary order by array_position(i.indkey,a.attnum)
  loop out := out || jsonb_build_object(r.attname,_row->r.attname); end loop;
  return out;
end $$;
...
DO $$ declare r record; begin
  for r in select n.nspname schema_name,c.relname table_name from pg_class c join pg_namespace n on n.oid=c.relnamespace
  where c.relkind='r' and n.nspname='public' and c.relname not in ('anaira_sync_events','anaira_sync_state')
