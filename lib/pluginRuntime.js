...
import { decryptMarketingToken } from "./marketingMeta"
...
export const PLUGIN_CONFIG_META = {
  ...
  "whatsapp-marketing": { required:["phone_number_id","access_token"], secrets:["access_token"] },
...
async function metaTest(config, code){
  if (config?.access_token) config = { ...config, access_token: decryptMarketingToken(config.access_token) }
...
export async function testPlugin({restaurantId, pluginCode, config = {}}){
  ...
  if (["whatsapp-invoice","whatsapp-marketing","facebook-integration","instagram-integration"].includes(pluginCode)) return metaTest(config,pluginCode)
