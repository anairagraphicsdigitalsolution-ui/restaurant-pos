...
export function printHtmlInFrame(html, { title = "Print", width = "80mm", height = "auto" } = {}) {
  if (typeof window !== "undefined" && window.anairaElectron?.previewPrint) {
    return window.anairaElectron.previewPrint({ html, title, width, height }).then((result) => {
      if (!result?.success) throw new Error(result?.error || "Unable to create PDF preview")
      return result
    })
  }
