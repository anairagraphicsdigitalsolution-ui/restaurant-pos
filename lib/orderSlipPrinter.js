...
export async function printOrderSlip(orderId, restaurantId) {
  ...
  try {
    ...
    if (job?.id) await supabaseCloudAdmin.from("print_jobs").update({ status: "printed", printed_at: new Date().toISOString(), last_error: null }).eq("id", job.id)
    if (kot?.kot_no) await supabaseCloudAdmin.from("kot_tickets").update({ printed_at: new Date().toISOString() }).eq("restaurant_id", restaurantId).eq("order_id", orderId)
    return { attempted: true, printed: true, result }
  } catch (error) {
    if (job?.id) await supabaseCloudAdmin.from("print_jobs").update({ status: "failed", attempts: 1, last_error: String(error?.message || error) }).eq("id", job.id)
