...
function getHeader(headers, configured, provider) {
  const names = [configured, ...(DEFAULT_HEADERS[String(provider || "").toLowerCase()] || [])].filter(Boolean)
...
export function verifyAggregatorWebhook({ provider, rawBody, headers, credentials }) {
  ...
  const suppliedRaw = getHeader(headers, signatureHeader, provider)
