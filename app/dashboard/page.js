...
function statusMeta(status) {
  ...
  if (["done", "completed", "served", "paid"].includes(key)) {
    return {
      label: "Completed",
      cls: "success"
    }
  ...
  if (["preparing", "in_kitchen", "in-kitchen"].includes(key)) {
    return {
      label: "Preparing",
      cls: "warning"
    }
...
export default function Dashboard() {
  ...
  async function loadData(rid) {
    ...
    try {
       ...
       * TOP SELLING
       *
       * Use actual order item prices where available.
       * Do not calculate historical sales using the current menu price.
       */
      ...
      for (const oi of payload.orderItems || []) {
        ...
        if (cancelledStatuses.has(orderStatus)) {
          continue
        }
       ...
       * LAST 7 DAYS SALES
       *
       * Keep the same cancelled-status logic.
       * Do not declare cancelledStatuses again here.
       */
      const days = []
  ...
  async function deleteItem(id) {
    if (
      ...
      return alert(
        "Only admin is allowed to delete menu items."
      )
    }
  ...
  async function deleteCategory(category) {
    if (
      ...
      return alert(
        "Only admin is allowed to delete categories."
      )
    }
  ...
  async function deleteAllOrders() {
    if (
      ...
      return alert("Only admin is allowed.")
    }

    if (
      !confirm(
        "⚠️ Delete ALL orders for this restaurant? This cannot be undone."
      )
    ) {
      return
    }
   ...
   * TODAY'S ORDERS
   *
   * Local list is still kept for recent/dashboard fallback.
   */
   ...
   * IMPORTANT:
   *
   * The API summary is the source of truth for today's KPI count.
   * This fixes the situation where Sales/Avg Bill had real values
   * but Today's Orders showed 0.
   */
  const todayOrderCount = Number(
      ...
      <div className="dashShell">
        ...
        <section className="statsGrid">
          ...
          <StatCard
            ...
            note={`${readyOrders} ready for service`}
          />
        ...
        <section className="mainGrid">
          ...
          <div className="panel">
            <PanelHeader
              title="Order Flow"
              subtitle="Current operational queue"
            />
        ...
        <section className="threeGrid">
          ...
          <div className="panel">
            ...
            <div className="listScroll">
              ...
              {!topSelling.length && (
                <Empty text="Sales data will appear here." />
              )}
            </div>
          </div>
          ...
          <div className="panel">
            <PanelHeader
              ...
              subtitle={`${todayReservations.length} bookings`}
              action={
                <button
                  className="textBtn"
                  onClick={() =>
            ...
            <div className="listScroll">
              ...
              {!todayReservations.length && (
                <Empty text="No reservations for today." />
              )}
            </div>
          </div>
        ...
        <section className="twoGrid">
          ...
          <div className="panel">
            <PanelHeader
              title="Quick Actions"
              subtitle="Jump directly into daily work"
            />
          ...
          <div className="panel galleryPanel">
            ...
            {items.length ? (
              ...
              <Empty text="Add menu items to show your spotlight." />
            )}
          </div>
      ...
      <style jsx global>{`
        ...
        .heroCard{
          ...
          background:
            linear-gradient(
              135deg,
              rgba(var(--surface-rgb),.96),
              rgba(var(--surface-2-rgb),.9)
            );
