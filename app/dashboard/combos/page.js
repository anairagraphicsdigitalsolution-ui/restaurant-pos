...
export default function CombosPage() {
  ...
  function editCombo(combo) {
    ...
    setSelectedOptions((cfg.groups?.[0]?.options || []).map(o => o.item_id))
    window.scrollTo({ top: 0, behavior: "smooth" })
  }

  function reset() {
    setEditingId(""); setForm(emptyForm); setSelectedItems({}); setSelectedOptions([]); setMessage("")
  }
  ...
  async function save() {
    ...
    const comboConfig = form.mode === "fixed"
      ...
      : { mode: "choice", groups: [{ name: form.groupName.trim() || "Choose an option", min: Number(form.min || 1), max: Number(form.max || 1), options: selectedOptions.map(item_id => ({ item_id, price_delta: 0 })) }] }
  ...
  return <main className="combo-page">
    ...
    <section className="builder">
      ...
      {form.mode === "fixed" ? <div className="item-grid">{filteredItems.map(item => <button className={`item-pick ${selectedItems[item.id] ? "selected" : ""}`} key={item.id} onClick={() => toggleFixed(item.id)}><span>{selectedItems[item.id] ? "✓" : "＋"}</span><div><b>{item.name}</b><small>{item.category || "Other"} · ₹{item.price}</small></div></button>)}</div> : <div className="choice-box"><div className="choice-fields"><input value={form.groupName} onChange={e => setForm({...form,groupName:e.target.value})} placeholder="Choice group name" /><input type="number" min="1" value={form.min} onChange={e => setForm({...form,min:e.target.value})} placeholder="Min" /><input type="number" min="1" value={form.max} onChange={e => setForm({...form,max:e.target.value})} placeholder="Max" /></div><div className="item-grid">{filteredItems.map(item => <button className={`item-pick ${selectedOptions.includes(item.id) ? "selected" : ""}`} key={item.id} onClick={() => setSelectedOptions(prev => prev.includes(item.id) ? prev.filter(id => id !== item.id) : [...prev, item.id])}><span>{selectedOptions.includes(item.id) ? "✓" : "＋"}</span><div><b>{item.name}</b><small>{item.category || "Other"} · ₹{item.price}</small></div></button>)}</div></div>}
