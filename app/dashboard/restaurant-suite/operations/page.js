...
export default function AnairaOperationsHub() {
  ...
  async function load(id = rid) {
    ...
    const q = {
      ...
      orders: supabaseCloud.from("orders").select("id,source_label,order_mode,status,total_amount,payment_status,table_id,created_at").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(40),
      discounts: supabaseCloud.from("discount_rules").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }),
      variants: supabaseCloud.from("menu_variants").select("*,menu_items(name)").eq("restaurant_id", id).order("name"),
      stations: supabaseCloud.from("kitchen_stations").select("*").eq("restaurant_id", id).order("sort_order").order("name"),
      kot: supabaseCloud.from("kds_events").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(30),
      riders: supabaseCloud.from("delivery_riders").select("*").eq("restaurant_id", id).order("name"),
      assignments: supabaseCloud.from("restaurant_deliveries").select("*").eq("restaurant_id", id).order("assigned_at", { ascending: false }).limit(30),
      tokens: supabaseCloud.from("order_tokens").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(30),
      waitlist: supabaseCloud.from("reservation_waitlist").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(30),
      reservations: supabaseCloud.from("reservations").select("id,name,phone,date,time,guests,status,table_id").eq("restaurant_id", id).order("date", { ascending: true }).limit(30),
      aggregators: supabaseCloud.from("aggregator_integrations").select("id,provider,outlet_code,active,last_sync_at").eq("restaurant_id", id),
      aggregatorOrders: supabaseCloud.from("aggregator_orders").select("*").eq("restaurant_id", id).order("received_at", { ascending: false }).limit(30),
      segments: supabaseCloud.from("customer_segments").select("*").eq("restaurant_id", id).order("name"),
      messages: supabaseCloud.from("message_queue").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(30),
      shifts: supabaseCloud.from("cash_shifts").select("*").eq("restaurant_id", id).order("opened_at", { ascending: false }).limit(10),
      reports: supabaseCloud.from("report_schedules").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }),
      prints: supabaseCloud.from("print_jobs").select("*").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(30),
      menu: supabaseCloud.from("menu_items").select("id,name,price").eq("restaurant_id", id).order("name").limit(200),
      plugins: supabaseCloud.from("restaurant_plugins").select("plugin_code,enabled").eq("restaurant_id", id),
      pluginSettings: supabaseCloud.from("plugin_settings").select("plugin_code,config").eq("restaurant_id", id).eq("plugin_code", "operations-hub"),
      feedback: supabaseCloud.from("customer_feedback").select("id,rating,feedback,created_at,customer_id,order_id").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(20),
      feedbackRequests: supabaseCloud.from("feedback_requests").select("id,order_id,customer_id,channel,status,created_at").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(20),
      deposits: supabaseCloud.from("reservation_deposits").select("id,reservation_id,amount,payment_method,status,paid_at").eq("restaurant_id", id).order("created_at", { ascending: false }).limit(20),
      expenses: supabaseCloud.from("expenses").select("id,category,description,amount,payment_method,expense_date,created_at").eq("restaurant_id", id).order("expense_date", { ascending: false }).limit(100),
      cashClosings: supabaseCloud.from("cash_closings").select("id,business_date,opening_cash,cash_sales,cash_in,cash_out,expense_cash,refunds,expected_cash,actual_cash,difference,notes,closed_at").eq("restaurant_id", id).order("business_date", { ascending: false }).limit(30),
      cashPayments: supabaseCloud.from("order_payments").select("amount,paid_at,created_at,status,payment_method").eq("restaurant_id", id).eq("payment_method", "cash").eq("status", "paid").limit(1000),
      cashRefunds: supabaseCloud.from("order_refunds").select("amount,created_at,status").eq("restaurant_id", id).eq("status", "refunded").limit(1000),
      branches: supabaseCloud.from("restaurant_branches").select("id,name,code,active,phone,address").eq("parent_restaurant_id", id).order("name"),
  ...
  return <main className="pp-wrap">
    ...
    <style jsx global>{`
      .pp-wrap{min-height:100vh;padding:28px;max-width:1500px;margin:0 auto;color:var(--text,var(--text));background:var(--background,var(--background))}
      .pp-hero{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding:28px;border:1px solid var(--border,rgba(255,255,255,.12));border-radius:24px;background:var(--surface,var(--surface));box-shadow:0 20px 70px rgba(0,0,0,.2)}
      .pp-hero small{color:var(--primary,var(--primary));letter-spacing:.18em;font-weight:800}.pp-hero h1{margin:8px 0;font-size:clamp(28px,4vw,48px);font-family:Georgia,serif}.pp-hero p,.pp-muted{color:var(--muted,var(--muted))}.pp-badge{padding:10px 14px;border-radius:999px;background:rgba(231,174,57,.1);border:1px solid rgba(231,174,57,.35);color:var(--primary,var(--primary));white-space:nowrap}
      .pp-tabs{display:flex;gap:8px;overflow:auto;padding:16px 0}.pp-tabs button,.pp-button,.pp-actions button,.pp-card button{border:1px solid var(--border,rgba(255,255,255,.12));background:var(--surface-2,var(--surface-2));color:var(--text,var(--text));border-radius:12px;padding:10px 14px;font-weight:700;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center}.pp-tabs button.active,.pp-card button:hover,.pp-actions button:hover,.pp-button:hover{background:var(--primary,var(--primary));color:#111}.pp-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.pp-card{border:1px solid var(--border,rgba(255,255,255,.12));background:var(--surface,var(--surface));border-radius:22px;padding:20px;min-width:0}.pp-card-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.pp-card h2{margin:0 0 16px;font-size:20px}.pp-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.pp-field{display:flex;flex-direction:column;gap:7px}.pp-field span{font-size:12px;color:var(--muted,var(--muted));font-weight:700}.pp-field input,.pp-field select,.pp-field textarea{width:100%;box-sizing:border-box;padding:11px 12px;border-radius:11px;border:1px solid var(--border,rgba(255,255,255,.12));background:rgba(0,0,0,.15);color:var(--text,var(--text));outline:none}.pp-form-grid button{align-self:end}.pp-list{margin-top:14px;display:flex;flex-direction:column}.pp-row{display:grid;grid-template-columns:1fr auto auto;gap:12px;align-items:center;padding:12px 0;border-top:1px solid var(--border,rgba(255,255,255,.08))}.pp-row span{color:var(--muted,var(--muted));font-size:13px}.pp-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.pp-table-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px;margin-top:16px}.pp-table{min-height:92px;display:flex!important;flex-direction:column;gap:5px;align-items:flex-start!important}.pp-table small,.pp-table em{font-size:12px;color:var(--muted,var(--muted));font-style:normal}.pp-table.occupied{border-color:#d45b4d}.pp-table.reserved{border-color:var(--primary)}.pp-empty,.pp-lock{padding:18px;border:1px dashed var(--border,rgba(255,255,255,.18));border-radius:16px;color:var(--muted,var(--muted))}.pp-lock{margin-bottom:16px;color:var(--primary)}.pp-stats{font-size:12px;color:var(--muted,var(--muted))}.pp-check{display:flex;align-items:center;gap:8px}.pp-link{display:inline-block;margin-top:16px;color:var(--primary,var(--primary))}.pp-toast{position:fixed;right:20px;bottom:20px;max-width:420px;padding:14px 18px;border:1px solid rgba(231,174,57,.4);background:var(--surface-2);color:var(--text);border-radius:14px;box-shadow:0 15px 40px rgba(0,0,0,.35);z-index:50}
      .feature-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:12px}.feature-card{padding:16px;border:1px solid var(--border,rgba(255,255,255,.12));border-radius:16px;background:rgba(0,0,0,.12)}.feature-card span{font-size:24px;display:block;margin-bottom:8px}.feature-card b{display:block}.feature-card p{margin:6px 0 0;color:var(--muted,var(--muted));font-size:13px;line-height:1.45}
