...
export default function RestaurantSuite() {
  ...
  return <main className="suite">
    ...
    {tab==="staff"&&<Panel title="Captain / Staff"><p className="muted">Captain sessions are device-aware. Connect staff mobile order screens to the same restaurant scope.</p>{captains.map(c=><div className="row" key={c.id}><b>{c.staff_name||"Staff"}</b><span>{c.device_name||"Device"} • {new Date(c.last_seen_at).toLocaleString("en-IN")}</span></div>)}{!captains.length&&<Empty text="No active captain sessions."/>}</Panel>}
    ...
    {tab==="reports"&&<Panel title="Reports & Exports"><div className="reportGrid">{["sales","orders","payments","inventory","food_cost","online_reconciliation","staff","customers","delivery_settlement","tax"].map(x=><button key={x} onClick={()=>requestExport(x)}>{x.replaceAll("_"," ")} → CSV</button>)}</div>{exports.map(x=><div className="row" key={x.id}><b>{x.report_type}</b><span>{x.status} • {new Date(x.created_at).toLocaleString("en-IN")}</span></div>)}</Panel>}
