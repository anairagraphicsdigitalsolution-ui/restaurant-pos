...
export default function PayPage(){
 ...
 return <main style={shell}><div style={wrap}>
   <header style={head}><div><small style={eyebrow}>SECURE QR BILL</small><h1 style={{margin:"5px 0"}}>{data?.restaurant?.name||"Restaurant"}</h1><p style={{margin:0}}>Order <b>#{orderNo}</b></p></div><div style={amount}>{money(due)}<span>Amount Due</span></div></header>
   <section style={card}><h2 style={sectionTitle}>Bill Summary</h2>{(data?.order_items||data?.items||[]).map((x,i)=><div key={x.id||i} style={row}><span>{x.item_name||x.name} × {x.quantity||1}</span><b>{money(x.line_total||Number(x.unit_price||0)*Number(x.quantity||1))}</b></div>)}<div style={row}><span>Subtotal</span><b>{money(order?.subtotal)}</b></div>{Number(order?.discount_amount)>0&&<div style={row}><span>Discount</span><b>-{money(order.discount_amount)}</b></div>}<div style={{...row,fontSize:20,borderTop:"1px solid var(--border)",paddingTop:14}}><strong>Total</strong><strong>{money(order?.total_amount)}</strong></div>{Number(order?.paid_amount)>0&&<div style={row}><span>Already paid</span><b>{money(order.paid_amount)}</b></div>}</section>
   {due>0&&<section style={card}><div style={paymentTitle}><div><small style={eyebrow}>MANUAL / UPI PAYMENT</small><h2 style={{margin:"4px 0"}}>Pay Bill</h2></div><span style={dueBadge}>{money(due)} due</span></div><p style={{color:"var(--muted)",marginTop:0}}>Use the restaurant QR below or scan any UPI QR with the scanner.</p>
     {request?.qr_image_url?<img src={request.qr_image_url} alt="Restaurant payment QR" style={qr}/>:request?.upi_uri?<div style={qrBox}><QRCode value={request.upi_uri} size={210}/></div>:<button style={primary} disabled={busy} onClick={start}>{busy?"Loading…":"Show Restaurant Payment QR"}</button>}
     <div style={payActions}><button style={secondaryHalf} onClick={()=>setScanner(true)}>📷 Scan UPI QR</button>{activeUpi&&<button style={secondaryHalf} onClick={openUpi}>Open UPI App</button>}</div>{activeUpi&&scannedUpi&&<div style={detected}>✓ UPI QR detected. Amount {money(due)} will be requested when supported by the payment app.</div>}
