...
export async function POST(request) {
  try {
    ...
    const requestedFloor = String(body?.floor || "").trim()
    ...
    let resolvedFloor = requestedFloor
    ...
    if (floorRows?.length) {
      const { data: validFloor } = await supabaseCloudAdmin
        .from("floors").select("id,name").eq("restaurant_id", restaurantId).eq("active", true).ilike("name", resolvedFloor).limit(1).maybeSingle()
      if (!validFloor) return NextResponse.json({ success: false, error: "Selected floor is not available." }, { status: 400 })
      resolvedFloor = validFloor.name
    }
    ...
    const { data, error } = await supabaseCloudAdmin
      .from("rooms")
      .insert({ room_number: value, restaurant_id: restaurantId, floor: resolvedFloor })
      .select("*")
      .single()
