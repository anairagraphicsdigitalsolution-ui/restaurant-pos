...
export async function POST(req){
  try {
    ...
    const id=b.id||crypto.randomUUID(); const items=Array.isArray(b.items)?b.items:[]; const status="pending"; const subtotal=Number(b.subtotal||0), discount=Number(b.discount_amount||0), tax=Number(b.tax_amount||0), total=Number(b.total_amount ?? subtotal-discount+tax); const createdAt=b.created_at||new Date().toISOString(); await localSql(`BEGIN; INSERT INTO orders (id,source_type,source_id,status,created_at,restaurant_id,source_label,overall_note,subtotal,discount_amount,tax_amount,total_amount,payment_status,invoice_no,sync_status,offline_created_at,updated_at) VALUES (${sqlText(id)},${sqlText(b.source_type||"table")},${sqlText(b.source_id||"")},${sqlText(status)},${sqlText(createdAt)},${sqlText(rid)},${sqlText(b.source_label||null)},${sqlText(b.overall_note||null)},${subtotal},${discount},${tax},${total},'unpaid','PENDING','pending',${sqlText(createdAt)},now()) ON CONFLICT (id) DO NOTHING; ${items.map((x,i)=>`INSERT INTO order_items (id,order_id,item_id,quantity,cooking_request,item_name,unit_price,line_total) VALUES (${sqlText(x.id||crypto.randomUUID())},${sqlText(id)},${sqlText(x.item_id||null)},${Number(x.quantity||1)},${sqlText(x.cooking_request||null)},${sqlText(x.item_name||x.name||"Item")},${Number(x.unit_price||0)},${Number(x.line_total||0)});`).join(" ")} INSERT INTO local_sync_outbox(entity,entity_id,operation,restaurant_id,payload) VALUES ('orders',${sqlText(id)},'upsert',${sqlText(rid)},${sqlJson({id,...b,status,subtotal,discount_amount:discount,tax_amount:tax,total_amount:total,invoice_no:'PENDING',sync_status:'pending',offline_created_at:createdAt})}) ON CONFLICT DO NOTHING; COMMIT;`); const order=await localOne(`SELECT * FROM orders WHERE id=${sqlText(id)}`); return Response.json({success:true,order}) } catch(e){ try{await localSql("ROLLBACK;")}catch{} return Response.json({success:false,error:e.message},{status:400}) }
}
