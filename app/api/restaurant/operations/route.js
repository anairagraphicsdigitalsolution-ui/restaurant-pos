...
export async function POST(req){
  try{
    ...
    if(["hold","park"].includes(action)){
      ...
      const {error}=await supabaseAdmin.from("orders").update({hold_status:holdType,status:"pending"}).eq("id",body.order_id).eq("restaurant_id",rid)
      if(error) throw error
      await supabaseAdmin.from("order_holds").insert({restaurant_id:rid,order_id:body.order_id,hold_type:holdType,note:body.note||null,created_by:user.id})
      return NextResponse.json({success:true,action})
    }

    if(action==="reopen"){
      const {error}=await supabaseAdmin.from("orders").update({hold_status:"active",reopened_at:new Date().toISOString(),void_reason:null}).eq("id",body.order_id).eq("restaurant_id",rid)
      if(error) throw error
    ...
    if(action==="refund"){
      ...
      const {data:payment}=await supabaseAdmin.from("order_payments").select("id").eq("restaurant_id",rid).eq("order_id",body.order_id).eq("status","paid").order("paid_at",{ascending:false}).limit(1).maybeSingle()
      ...
      if(error) throw error
      if(payment?.id) await supabaseAdmin.from("order_payments").update({status:"refunded"}).eq("id",payment.id)
      return NextResponse.json({success:true})
    }
    ...
    if(action==="payment"){
      ...
      const {error}=await supabaseAdmin.from("order_payments").insert({restaurant_id:rid,order_id:body.order_id,payment_method:method,amount,reference:body.reference||null,status:"paid",created_by:user.id})
      if(error) throw error
      return NextResponse.json({success:true})
    }
    ...
    if(action==="merge"){
      ...
      const {data:rows,error}=await supabaseAdmin.from("orders").select("id,total_amount").eq("restaurant_id",rid).in("id",ids)
      if(error) throw error
      const total=(rows||[]).reduce((n,x)=>n+Number(x.total_amount||0),0)
      const [target,...rest]=rows||[]
      if(!target) throw new Error("Orders not found")
      await supabaseAdmin.from("orders").update({total_amount:total}).eq("id",target.id).eq("restaurant_id",rid)
      if(rest.length) await supabaseAdmin.from("orders").update({status:"cancelled",void_reason:`Merged into ${target.id}`}).in("id",rest.map(x=>x.id)).eq("restaurant_id",rid)
    ...
    if(action==="transfer_table"){
      ...
      const {data:order,error}=await supabaseAdmin.from("orders").select("source_id").eq("id",order_id).eq("restaurant_id",rid).single()
      if(error) throw error
      const {error:updateError}=await supabaseAdmin.from("orders").update({source_type:"table",source_id:to_table_id,source_label:body.source_label||null}).eq("id",order_id).eq("restaurant_id",rid)
      if(updateError) throw updateError
      await supabaseAdmin.from("order_transfers").insert({restaurant_id:rid,order_id,from_table_id:order.source_id,to_table_id,moved_by:user.id})
      return NextResponse.json({success:true})
    }
    ...
    if(action==="move_items"){
      ...
      const {data:order}=await supabaseAdmin.from("orders").select("source_id").eq("id",body.order_id).eq("restaurant_id",rid).single()
      await supabaseAdmin.from("order_item_moves").insert({restaurant_id:rid,order_item_id:body.order_item_id,order_id:body.order_id,from_table_id:order?.source_id||null,to_table_id:body.to_table_id,quantity:Number(body.quantity||1),moved_by:user.id})
      return NextResponse.json({success:true})
    }
    ...
    if(action==="kds"){
      ...
      const {error}=await supabaseAdmin.from("kitchen_order_tickets").insert(patch)
      if(error) throw error
