...
export async function GET(req){
  ...
  try{
    ...
    const [{data:plugin},{data:settings},{data:offers}] = await Promise.all([
      supabaseAdmin.from("restaurant_plugins").select("enabled").eq("restaurant_id",restaurant.id).eq("plugin_code","offers").maybeSingle(),
      supabaseAdmin.from("plugin_settings").select("config").eq("restaurant_id",restaurant.id).eq("plugin_code","offers").maybeSingle(),
      supabaseAdmin.from("offers").select("*").eq("restaurant_id",restaurant.id).eq("active",true)
    ])
