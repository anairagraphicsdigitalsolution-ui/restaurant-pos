...
async function authContext(req,restaurantId){
  ...
  const {data:profile,error}=await supabaseCloudAdmin.from("profiles").select("id,role,restaurant_id")
...
export async function POST(req){
  try{
    ...
    const {data:settings,error}=await supabaseCloudAdmin.from("plugin_settings").select("config")
