...
export async function GET(req) {
  try {
    ...
    const [{ data: plans, error: planError }, { data: subscriptions, error: subError }, { data: restaurants, error: restError }] = await Promise.all([
      supabaseAdmin.from("saas_plans").select("*").order("monthly_price"),
      supabaseAdmin.from("restaurant_subscriptions").select("*").order("updated_at", { ascending: false }),
      supabaseAdmin.from("restaurants").select("id,name,status,owner_name,phone").order("name")
    ])
...
export async function POST(req) {
  try {
    ...
    const { data: restaurant, error: restError } = await supabaseAdmin.from("restaurants").select("id,name,status").eq("id", restaurantId).maybeSingle()
    ...
    if (saasPlanId) {
      const { data: plan, error: planError } = await supabaseAdmin.from("saas_plans").select("id,active").eq("id", saasPlanId).maybeSingle()
    ...
    const { data: existing } = await supabaseAdmin.from("restaurant_subscriptions").select("id").eq("restaurant_id", restaurantId).order("updated_at", { ascending: false }).limit(1).maybeSingle()
    let subscription
    if (existing?.id) {
      const { data, error } = await supabaseAdmin.from("restaurant_subscriptions").update({ saas_plan_id: saasPlanId || null, status, billing_cycle: billingCycle, starts_at: normalizedAction === "approve" ? now : null, ends_at: endsAt, updated_at: now }).eq("id", existing.id).select("*").single()
      if (error) throw error
      subscription = data
    } else {
      const { data, error } = await supabaseAdmin.from("restaurant_subscriptions").insert({ restaurant_id: restaurantId, saas_plan_id: saasPlanId || null, status, billing_cycle: billingCycle, starts_at: normalizedAction === "approve" ? now : null, ends_at: endsAt, updated_at: now }).select("*").single()
      if (error) throw error
      subscription = data
    }
    const restaurantStatus = normalizedAction === "approve" ? "active" : "inactive"
    const { error: statusError } = await supabaseAdmin.from("restaurants").update({ status: restaurantStatus }).eq("id", restaurantId)
