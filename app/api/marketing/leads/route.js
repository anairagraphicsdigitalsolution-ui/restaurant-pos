...
async function ctx(req){const user=await requireApiUser(req);const r=await resolveRestaurantForUser(user);if(!r.restaurantId)throw new Error("Restaurant not found");await requireFeature(r.restaurantId,"p1-marketing-hub");await requireStaffPermission(user,r.restaurantId,"marketing");const {data:rows}=await supabaseCloudAdmin.from("restaurant_plugins").select("enabled").eq("restaurant_id",r.restaurantId).in("plugin_code",["facebook-integration","instagram-integration","whatsapp-marketing"]);if(!(rows||[]).some(x=>x.enabled===true))throw new Error("Marketing is not active for this restaurant");return {user,restaurantId:r.restaurantId}}
