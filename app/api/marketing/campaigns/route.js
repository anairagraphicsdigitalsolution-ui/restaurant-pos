...
async function ctx(req){const user=await requireApiUser(req);const r=await resolveRestaurantForUser(user);if(!r.restaurantId)throw new Error("Restaurant not found");await requireFeature(r.restaurantId,"p1-marketing-hub");await requireStaffPermission(user,r.restaurantId,"marketing");const enabled=await marketingEnabled(r.restaurantId);if(!enabled)throw new Error("Marketing is not active for this restaurant");return {user,restaurantId:r.restaurantId}}
